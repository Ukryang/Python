...
def reservation_date(club):
    ...
    header = {
        "Referer": "https://booking.hanwharesort.co.kr/pzc/pnr/0010/serviceM00.mvc?authKey=9c87099c2a4d44b4b6bae39a8bde0cc1",
...
def reservation_time(BRCH_CD, li_number):
    ...
    return RSRV_DATE, RSRV_MONTH, RSRV_TIME
...
if __name__ == "__main__":
    ...
    RSRV_DATE, RSRV_MONTH, RSRV_TIME = reservation_time(BRCH_CD, li_number)
    ...
    tr_length = len(driver.find_elements_by_xpath('//table[1]/tbody/tr'))
    tr_count = 2

    while tr_count < tr_length + 1:
        try:
            srsvtime = driver.find_element_by_xpath('//table[1]/tbody/tr[{}]/td[5]/a'.format(tr_count)).get_attribute('srsrvtime')
            if RSRV_TIME == srsvtime:
                driver.find_element_by_xpath('//table[1]/tbody/tr[{}]/td[5]/a'.format(tr_count)).click()
                break
            tr_count += 1
        except:
            break
